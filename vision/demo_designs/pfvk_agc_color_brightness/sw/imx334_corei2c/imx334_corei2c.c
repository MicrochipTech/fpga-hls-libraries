// https://kernel.googlesource.com/pub/scm/linux/kernel/git/tip/tip/+/refs/tags/x86_misc_for_v6.1_rc1/drivers/media/i2c/imx334.c
...
#ifdef USE_UART
extern UART_instance_t gUart;
extern uint8_t gTxBuf[UART_BUF_SZ];
#endif
...
static i2c_status_t sensor_i2c_write(uint8_t i2c_ch_sel, uint16_t data_reg, uint8_t data) {
    ...
    if (status != I2C_SUCCESS) {
        PRINTF("ERROR. sensor_i2c_write> st:%d\r\n", status);
    }
    return status;
}
...
//     msdelay(100);
//     I2C_enable_slave(sensor1_i2c);
//     I2C_enable_slave(sensor2_i2c);
// }
...
void imx334_cam_init(void)
{
    ...
    // GPIO_set_output(&g_gpio_out, CAM1_RST, 0u);
    GPIO_set_output(&g_gpio_out, CAM2_RST, 0u);
    GPIO_set_output(&g_gpio_out, CAM2_CLK_EN, 0u);
    msdelay(100);
    // GPIO_set_output(&g_gpio_out, CAM1_RST, 1u); // Bring camera out of reset
    GPIO_set_output(&g_gpio_out, CAM2_RST, 1u); // Bring camera out of reset
    msdelay(100);
    GPIO_set_output(&g_gpio_out, CAM2_CLK_EN, 1u); //Enable Cam clock from FPGA
    msdelay(100);
