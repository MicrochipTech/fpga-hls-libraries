...
//    float delta = (2 * M_PI) / 256 / 1024;
    ...
    for (double x = start; x < limit; x += delta, count++) {
        double actual_lut_16 = wrapper_lut_16(x);
        double actual_lut_24 = wrapper_lut_24(x);
        double actual_lut_28 = wrapper_lut_28(x);
  ...
  //    printf("%f,%lf,%lf,%lf,%lf,%lf,%lf\n", x, expect, actual_cordic, actual_taylor, actual_lut_16, actual_lut_24, actual_lut_28);
    }
